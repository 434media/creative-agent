...
export default function Home() {
  ...
  const handleSubmit = async () => {
    ...
    setImage(null);
    setCaption("");
    const prompt = `${title}. ${details} — ${date} at ${location}`;
        ...
        <div className="flex-1">
          ...
          <div className="border border-white/40 bg-white/10 p-4 rounded mb-6">
            <input
              ...
              placeholder="Title (e.g. Summer Zine Fair)"
            />
            <input
              className="border p-2 w-full mb-4 rounded text-white"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              placeholder="Date (e.g. August 12, 2025)"
            />
            <input
              className="border p-2 w-full mb-4 rounded text-white"
              value={location}
              onChange={(e) => setLocation(e.target.value)}
              placeholder="Location (e.g. 434 Gallery, Providence)"
            />
            <textarea
              className="border p-2 w-full mb-4 rounded text-white resize-none h-20"
              value={details}
              onChange={(e) => setDetails(e.target.value)}
              placeholder="Additional details (e.g. Risograph printing, local vendors, open submissions...)"
            />
